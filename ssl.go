...
const CertPath = "/Users/yuchi/Dropbox/RogersAI/Develop/mqtt-go/mosquitto"
...
func NewTLSConfig() *tls.Config {
	...
	pemCerts, err := ioutil.ReadFile(CertPath + "/cert/ca-cert.pem")
	if err == nil {
		certpool.AppendCertsFromPEM(pemCerts)
	}

	// Import client certificate/key pair
	cert, err := tls.LoadX509KeyPair(CertPath+"/cert/client-cert.pem",
		CertPath+"/cert/client-key.pem")
	if err != nil {
		panic(err)
	}
...
func sslMain() {
	...
	c.Subscribe("/go-mqtt/sample-ssl", 0, nil)
	...
	for range time.Tick(time.Duration(1) * time.Second) {
		...
		c.Publish("/go-mqtt/sample-ssl", 0, false, text)
		i++
	}
